...
class QCJSAPI {
    ...
    static appCallback(options) {
        ...
        const { callbackid, data: response } = result;
        ...
        if (callback && isFunction(callback)) {
            ...
            if (/getsessionid/i.test(callbackid)) {
                if (data instanceof Object) {
                    const temporarys = Object.entries(data);
                    const recombine = {};
                    temporarys.map(temporary => {
                        let [key, value] = temporary; // eslint-disable-line prefer-const
                        if (/sessionid/i.test(key)) {
                            key = String(key).toLowerCase();
                        }
                        recombine[key] = value;
                    });
                    response.data = recombine;
                }
            }
            if (/faceplusplus/i.test(callbackid)) {
                if (data && data.ocr) {
                    data.ocr = translate(data.ocr);
                    response.data = data;
                }
            }
            callback(response);
